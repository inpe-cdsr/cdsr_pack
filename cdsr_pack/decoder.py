...
def decode_scene_dir(scene_dir):
    ...
    if scene_dir_first.startswith('AMAZONIA_1') or scene_dir_first.startswith('CBERS_4'):
        ...
        if len(time) >= 3:
            ...
            raise CDSRDecodeException(f'Invalid spplited time: `{scene_dir_second}`.')
        ...
        if len(time) != 6:
            ...
            raise CDSRDecodeException(f'Size of `{time}` time is not 6.')

        # I build the time string based on the old one (e.g. from '145654' to '14:56:54')
        time = f'{time[0:2]}:{time[2:4]}:{time[4:6]}'

    else:
        raise CDSRDecodeException(f'Invalid scene directory: `{scene_dir}`.')
...
def decode_path_row_dir(path_row_dir):
    ...
    if len(splitted_path_row) == 3:
        ...
        raise CDSRDecodeException(f'`{path_row_dir}` path/row directory cannot be decoded.')
...
def decode_geo_processing_dir(geo_processing_dir):
    ...
    raise CDSRDecodeException(f'`{geo_processing_dir}` geo. processing '
